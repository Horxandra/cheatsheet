// FOR LOOP
for (let number = 1; number < 100; number++) { //IF I want this block to run 100 times
...
for (const name of myString) { //
  //THIS block will run 'multiple times' - 4 times 
// in this case
  // value of 'name' variable will be: "o" (first 
// time), "t" (second time), "t" (third 
// time), "o" (fourth time) 
//variable for the loop should be descriptive 
}
...
for (const key in obj) {
    ...
    //value of "letter" variable will be: "name" (first time), "age" (second time) 
...
for (const index in myArray) {  // if we care about the current index
  ...
  const valueOfCurrentElem = myArray[index]
}
